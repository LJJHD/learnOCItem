...
+(BOOL)isBalancedBinaryTree:(BinaryTreeNode *)rootNode{
    NSInteger depth = 0;
    return [self isBalancedBinaryTreeCore:rootNode depth:depth];
}
+(BOOL)isBalancedBinaryTreeCore:(BinaryTreeNode *)rootNode depth:(NSInteger)depth{
    if (!rootNode) {
        return YES;
    }
    NSInteger left = 0;
    NSInteger right = 0;
    if ([self isBalancedBinaryTreeCore:rootNode.leftNode depth:left] && [self isBalancedBinaryTreeCore:rootNode.rightNode depth:right]) {
        NSInteger diff = left - right;
        if (diff >= -1 && diff <= 1) {
